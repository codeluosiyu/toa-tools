...
export const shortestPath = function (grid: Array<Array<number>>, k: number) {
    ...
    const dp = Array(height).fill(null).map((_) =>
        Array(width).fill(null).map((_) => Array(k + 1).fill(Infinity))
    );
    dp[0][0][0] = 0;
    const queue: Array<any> = [[0, 0]];
...
/** 数组乱序 */
export const arrScrambling = (arr: Array<any>) => {
 ...
 * @returns
 */
export const splits = (tranvalue: string) => {
    let value = new Array("", "");
