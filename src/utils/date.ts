 ...
 * @returns
 */
export const checkTimeStamp = (time: number) => {
    if (time < 90000000000) {
        time *= 1000;
    }
...
export const getDateDiff = (startTime: any, endTime: any) => {
    var sTime: any = new Date(startTime).getTime();
    var eTime: any = new Date(endTime).getTime();
    ...
    const hour: any = parseInt(`${((eTime - sTime) % parseInt(`${divNumDay}`)) / parseInt(`${divNumHour}`)}`);
    const minute: any = parseInt(`${parseInt(`${((eTime - sTime) % parseInt(`${divNumDay}`)) % parseInt(`${divNumHour}`)}`) /
